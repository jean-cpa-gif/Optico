...
export default function OperacoesAbertas() {
  ...
  const sugestoesFiltradas = todasOpcoesCadastradas.filter((ativo: string) => {
    ...
    return ativo.toLowerCase().includes(busca.toLowerCase()) && 
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex flex-wrap items-center gap-2.5">
            <button 
              ...
              className={`flex items-center gap-2 px-3.5 py-1.5 text-xs font-semibold rounded border transition-colors cursor-pointer shrink-0 ${
            ...
            {/* Caixa de pesquisa de ativo/opção com sugestões */}
            <div ref={searchRef} className="relative w-44 sm:w-56 shrink-0">
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                <input
                  id="search-ativo-input"
                  type="text"
                  placeholder="Buscar ativo/opção..."
                  value={busca}
                  onChange={(e) => {
                  ...
                  className="w-full pl-8 pr-7 py-1.5 text-xs font-medium bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-slate-700 dark:text-slate-300 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                />
                {busca && (
                  <button
              ...
              {showSugestoes && sugestoesFiltradas.length > 0 && (
                <div 
                  ...
                  className="absolute z-50 left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded shadow-lg py-1 animate-fade-in"
                >
                  <div className="px-2.5 py-1 text-[9px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-700/50 mb-1">
                    Sugestões
                  </div>
                  {sugestoesFiltradas.map((sugestao) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-1.5 text-xs font-semibold hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300 cursor-pointer transition-colors"
                    >
                      {sugestao}
                    </button>
                  ))}
            ...
            {activeFiltersCount > 0 && (
              <button
                ...
                className="text-xs font-medium text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 flex items-center gap-1 transition-colors cursor-pointer px-2 py-1 shrink-0"
              >
                Limpar filtros
              </button>
            )}
          ...
          {!showFilters && activeFiltersCount > 0 && (
            <div className="flex gap-1.5 flex-wrap">
              ...
              {busca && (
                <span className="px-2 py-0.5 rounded-full text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium border border-slate-200/50 dark:border-slate-700/50 flex items-center gap-1">
                  Ativo: {busca}
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
